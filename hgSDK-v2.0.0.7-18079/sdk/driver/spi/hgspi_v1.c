...
static int32 hgspi_v1_write(struct spi_device *p_spi, const void *buf, uint32 size) {
    ...
    os_mutex_lock(&dev->os_spi_tx_lock, osWaitForever);
    ...
    if (hw->CON0 & LL_SPI_CON0_SLAVE_STATE_EN) {
        ...
        hgspi_v1_spi_slave_fsm_set_tx_ready_sta(hw, size);

        os_mutex_unlock(&dev->os_spi_tx_lock);
...
static int32 hgspi_v1_read(struct spi_device *p_spi, void *buf, uint32 size) {
    ...
    os_mutex_lock(&dev->os_spi_rx_lock, osWaitForever);
    ...
    if (hw->CON0 & LL_SPI_CON0_SLAVE_STATE_EN) {
        ...
        hgspi_v1_spi_slave_fsm_set_rx_ready_sta(hw, size);

        os_mutex_unlock(&dev->os_spi_rx_lock);

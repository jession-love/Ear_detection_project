...
void delay_ms(uint32 n)
{
    ...
    if (n) {
        __delay_asm(((sys_clk/1000*n - 8)/3));
    }
}
...
bool usb_hub_get_status(struct hgusb20_dev *p_dev,uint8 *pinf)
{
    ...
    usb_host_ep0_request(p_dev, pinf);
    return FALSE;
}
...
void usb_host_hub_get_descriptor(struct hgusb20_dev *p_dev, uint16 len)
{
    p_dev->usb_ctrl.cmd.rtype = 0xa0;
    p_dev->usb_ctrl.cmd.request = 0x06;                 //Get Descriptor
    p_dev->usb_ctrl.cmd.value = 0;
    ...
    usb_host_ep0_request(p_dev, p_dev->usb_ep0buf);

}

bool usb_host_get_hub_descriptor(struct hgusb20_dev *p_dev,uint8* buf)
{
    usb_host_hub_get_descriptor(p_dev, 64);         //第一次，尝试获取设备描述符
    //p_dev->usb_host.ep0_pkt = p_dev->usb_ep0buf[7];                             //Max Pkt
...
{    
    hgusb20_host_rx_ep_init(p_dev, HUB_EP, hub_manage.int_endpoint, USB_ENDPOINT_XFER_ISOC, 4);
...
bool usb_hub_get_port_status(struct hgusb20_dev *p_dev,uint8 *desc,uint8 port)
{
    ...
    usb_host_ep0_request(p_dev, desc);
    return FALSE;
}
...
uint8 usb_host_ep0_enum_hub(struct hgusb20_dev *p_dev)
{
    ...
    for(i=1;i<hub_manage.port_num+1;i++){
        usb_hub_get_port_status(p_dev,hub_des,i);
...
void hub_wait_port_connect(struct hgusb20_dev *p_dev,uint8 port)
{
    uint8 buff[10];
    
    do{
        usb_hub_get_port_status(p_dev,buff,port);
        os_sleep_ms(1);    
    }while(!(buff[0] & HUB_STATU_CONNECT) );
}

void hub_wait_port_restend(struct hgusb20_dev *p_dev,uint8 port)
{
    uint8 buff[10];
    usb_hub_get_port_status(p_dev,buff,port);
    do{
...
//  if(ret != 0)return ret;
    ...
    hub_wait_port_connect(p_dev,port);  

    ret = hub_ep0_clear_port_feature(p_dev,port,CLEAR_PORT_REST);
    if(ret != 0)return ret;

    hub_wait_port_restend(p_dev,port);
    return ret;
...
//        res = usb_host_ep0_enum_uvc(usb_sw);
        ...
        if (res == 0) {
            ...
            usb_host_ep0_enum_hub_dev(p_dev);
        }

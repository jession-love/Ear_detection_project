...
struct sys_config sys_cfgs = {
	...
	.ipaddr 	   = IP_ADDR_DEFAULT,//0x0101A8C0, //192.168.1.1
	.netmask	   = NET_MASK_DEFAULT,//0x00FFFFFF, //255.255.255.0
	.gw_ip		   = GW_IP_DEFAULT,//0x0101A8C0, //192.168.1.1
	.dhcpd_startip = DHCP_START_IP_DEFAULT,//0x6401A8C0, //192.168.1.2
	.dhcpd_endip   = DHCP_END_IP_DEFAULT,//0xFE01A8C0, //192.168.1.254
...
int32 wificfg_atcmd_set_ssid(const char *cmd, char *argv[], uint32 argc)
{
    if (argc == 1) {
        ...
        os_strcpy(sys_cfgs.ssid, argv[0]);
#ifdef CONFIG_UMAC4
